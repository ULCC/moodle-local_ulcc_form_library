...
/*
 * much of the guts of this class inherited from form_element_plugin_itemlist
*/
class form_element_plugin_status extends form_element_plugin_itemlist{
    ...
    function __construct() {
    	...
    	$this->data_entry_tablename     = "ulcc_form_plg_sts_ent";
    	//$this->data_entry_tablename = "block_form_user_status";
	...
	*/
	public function config_specific_definition(&$mform) {
		...
		$info = $this->get_option_list_text( FORM_DEFAULT_USERSTATUS_RECORD , "\n", 'passfail' ) ;
		...
		$E = $mform->addElement(
			'textarea',
			'optionlist',
			get_string( 'form_element_plugin_dd_optionlist_additional', 'local_ulcc_form_library' ),
			array('class' => 'form_input')
	    );
	...
	*/
        ...
        //we now have 2 lists: $pass_list and $fail_list 
	 	...
	 	if (empty($plgrec)) {
			...
			//each option is one row
	 		$element_id = $this->dbc->get_form_element_record($this->tablename,$data);
			...
			foreach( $optionlist as $key=>$itemname ){
				...
				$itemrecord->name = $itemname;
	 			...
	 			$this->dbc->get_form_element_record($this->items_tablename,$itemrecord);
			}
	 	} else {
    ...
    public function install(){
        ...
        //this table records an instance creation of a status field in a form
        $tablename = "ulcc_form_plg_rf_sts";
        ...
        $table_uid = new $this->xmldb_field('formfield_id');
    ...
    function language_strings(&$string) {
        $string['form_element_plugin_status'] 			= 'Select';
        $string['form_element_plugin_status_type'] 		= 'status select';
        $string['form_element_plugin_status_description'] 	= 'A status selector';
		$string[ 'form_element_plugin_status_optionlist' ] 	= 'Option List';
		$string[ 'form_element_plugin_status_single' ] 		= 'Single select';
		$string[ 'form_element_plugin_status_multi' ] 		= 'Multi select';
		$string[ 'form_element_plugin_status_typelabel' ] 	= 'Select type (single/multi)';
    ...
    */
    ...
    public function audit_type() {
        return get_string('form_element_plugin_status_type','local_ulcc_form_library');
    }
    ...
    */
	public function get_option_list( $formfield_id, $field=false, $useid=true){
		...
		if( $formfield_id ){
			...
			$objlist = $this->dbc->listelement_item_exists( $this->items_tablename, array( 'parent_id' => FORM_DEFAULT_USERSTATUS_RECORD ) );
