...
class form_element_plugin_comment_editor_mform  extends form_element_plugin_mform {
	 ...
	 protected function specific_validation($data) {
	 	...
	 	if ($data->maximumlength < 0 || $data->maximumlength > 9999) $this->errors['maximumlength'] = get_string('form_element_plugin_comment_editor_maxlengthrange','local_ulcc_form_library');
	 	if ($data->maximumlength < $data->minimumlength) $this->errors['maximumlength'] = get_string('form_element_plugin_comment_editor_maxlessthanmin','local_ulcc_form_library');
	 ...
	 protected function specific_process_data($data) {
	 	...
	 	$plgrec = (!empty($data->formfield_id)) ? $this->dbc->get_form_element_record("ulcc_form_plg_cedt",$data->formfield_id) : false;
