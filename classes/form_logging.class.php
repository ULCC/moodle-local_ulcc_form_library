...
/**
 * Class to enable the logging of user actions in the whilst interacting with
 * the form class
 *
...
class form_logging {
     ...
     * add a entry into the log table
     *
    ...
    protected function update_record($table, $paramsobj) {
        ...
        if ($success) {
            $this->add_to_audit($table, FORM_LOG_UPDATE, $paramsobj, $currobject);
        }
    ...
    protected function insert_record($table, $paramsobj) {
        ...
        if ($paramsobj->id) {
            $this->add_to_audit($table, FORM_LOG_ADD, $paramsobj);
        }
    ...
    protected function delete_records($table, $params, $extraparams) {
        ...
        if (!empty($deleteobject)) {
            foreach ($deleteobject as $delobj) {
                ...
                $this->add_to_audit($table, FORM_LOG_DELETE, $delobj);
            }
        ...
        if( in_array( $table, $fetables ) )	{
	        ...
	        foreach ($newobject as $key => $val) {
	            if ((in_array($key,$attributes) &&
	                ...
	                $log->candidate_id = $USER->id;
	                $log->course_id = false;
                    $log->type = ( isset( $newobject->capability_id ) ) ? get_string( 'reportpermissions' , 'block_ilp' ) :
          " " . $oplist[ $action ];
